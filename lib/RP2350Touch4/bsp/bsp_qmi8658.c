...
{
    bsp_i2c_read_reg8(QMI8658_slave_addr, reg_addr, data, len);
}

static void bsp_qmi8658_reg_write_byte(uint8_t reg_addr, uint8_t *data)
{
    bsp_i2c_write_reg8(QMI8658_slave_addr, reg_addr, data, 1);
}

void bsp_qmi8658_config_acc(enum QMI8658_AccRange range, enum QMI8658_AccOdr odr, enum QMI8658_LpfConfig lpfEnable, enum QMI8658_StConfig stEnable)
{
	...
	bsp_qmi8658_reg_write_byte(QMI8658Register_Ctrl2, &ctl_dada);
	// set LPF & HPF
	bsp_qmi8658_reg_read(QMI8658Register_Ctrl5, &ctl_dada, 1);
	ctl_dada &= 0xf0;
	...
	ctl_dada = 0x00;
	bsp_qmi8658_reg_write_byte(QMI8658Register_Ctrl5, &ctl_dada);
	// set LPF & HPF
}

void bsp_qmi8658_config_gyro(enum QMI8658_GyrRange range, enum QMI8658_GyrOdr odr, enum QMI8658_LpfConfig lpfEnable, enum QMI8658_StConfig stEnable)
{
	...
	if (stEnable == QMI8658St_Enable)
		...
		ctl_dada = (unsigned char)range | (unsigned char)odr;
	bsp_qmi8658_reg_write_byte(QMI8658Register_Ctrl3, &ctl_dada);
...
void bsp_qmi8658_config_ae(enum QMI8658_AeOdr odr)
{
	// bsp_qmi8658_config_acc(QMI8658AccRange_8g, AccOdr_1000Hz, Lpf_Enable, St_Enable);
	// bsp_qmi8658_config_gyro(QMI8658GyrRange_2048dps, GyrOdr_1000Hz, Lpf_Enable, St_Enable);
...
unsigned char bsp_qmi8658_readStatus0(void)
{
	...
	bsp_qmi8658_reg_read(QMI8658Register_Status0, status, sizeof(status));
 ...
 */
unsigned char bsp_qmi8658_readStatus1(void)
{
	unsigned char status;

	bsp_qmi8658_reg_read(QMI8658Register_Status1, &status, sizeof(status));
...
float bsp_qmi8658_readTemp(void)
{
	...
	bsp_qmi8658_reg_read(QMI8658Register_Tempearture_L, buf, 2);
...
void bsp_qmi8658_read_acc_xyz(float acc_xyz[3])
{
	unsigned char buf_reg[6];
	short raw_acc_xyz[3];

	bsp_qmi8658_reg_read(QMI8658Register_Ax_L, buf_reg, 6); // 0x19, 25
...
void bsp_qmi8658_read_gyro_xyz(float gyro_xyz[3])
{
	unsigned char buf_reg[6];
	short raw_gyro_xyz[3];

	bsp_qmi8658_reg_read(QMI8658Register_Gx_L, buf_reg, 6); // 0x1f, 31
...
void bsp_qmi8658_read_data(qmi8658_data_t *data)
{
	...
	if (data)
	{
		...
		bsp_qmi8658_reg_read(QMI8658Register_Timestamp_L, buf, 3); // 0x18	24
		...
		data->tim_count = imu_timestamp;
	}

	bsp_qmi8658_reg_read(QMI8658Register_Ax_L, buf_reg, 12); // 0x19, 25
	raw_acc_xyz[0] = (short)((unsigned short)(buf_reg[1] << 8) | (buf_reg[0]));
	raw_acc_xyz[1] = (short)((unsigned short)(buf_reg[3] << 8) | (buf_reg[2]));
	raw_acc_xyz[2] = (short)((unsigned short)(buf_reg[5] << 8) | (buf_reg[4]));
	...
	// mg
	data->acc_x = (float)(raw_acc_xyz[AXIS_X] * 1000.0f) / acc_lsb_div;
	data->acc_y = (float)(raw_acc_xyz[AXIS_Y] * 1000.0f) / acc_lsb_div;
	data->acc_z = (float)(raw_acc_xyz[AXIS_Z] * 1000.0f) / acc_lsb_div;
#else
	// m/s2
	data->acc_x = (float)(raw_acc_xyz[AXIS_X] * ONE_G) / acc_lsb_div;
	data->acc_y = (float)(raw_acc_xyz[AXIS_Y] * ONE_G) / acc_lsb_div;
	data->acc_z = (float)(raw_acc_xyz[AXIS_Z] * ONE_G) / acc_lsb_div;
#endif
	//	data->acc_x = imu_map.sign[AXIS_X]*acc_t[imu_map.map[AXIS_X]];
	//	data->acc_y = imu_map.sign[AXIS_Y]*acc_t[imu_map.map[AXIS_Y]];
	//	data->acc_z = imu_map.sign[AXIS_Z]*acc_t[imu_map.map[AXIS_Z]];

#if defined(QMI8658_UINT_MG_DPS)
	// dps
	data->gyr_y = -((float)(raw_gyro_xyz[0] * 1.0f) / gyro_lsb_div);
	data->gyr_x = (float)(raw_gyro_xyz[1] * 1.0f) / gyro_lsb_div;
	data->gyr_z = (float)(raw_gyro_xyz[2] * 1.0f) / gyro_lsb_div;
#else
	// rad/s
...
void bsp_qmi8658_read_xyz_raw(short raw_acc_xyz[3], short raw_gyro_xyz[3], unsigned int *tim_count)
{
	unsigned char buf_reg[12];
...
// void bsp_qmi8658_read_ae(float quat[4], float velocity[3])
// {
// 	unsigned char buf_reg[14];
// 	short raw_q_xyz[4];
// 	short raw_v_xyz[3];

// 	bsp_qmi8658_reg_read(QMI8658Register_Q1_L, buf_reg, 14);
...
void bsp_qmi8658_doCtrl9Command(enum QMI8658_Ctrl9Command cmd)
{
	uint8_t val;
...
void bsp_qmi8658_enableWakeOnMotion(void)
{
	...
	bsp_qmi8658_enableSensors(QMI8658_CTRL7_DISABLE_ALL);
	bsp_qmi8658_config_acc(QMI8658AccRange_2g, QMI8658AccOdr_LowPower_21Hz, QMI8658Lpf_Disable, QMI8658St_Disable);
	...
	bsp_qmi8658_reg_write_byte(QMI8658Register_Cal1_L, &womCmd[1]);
	bsp_qmi8658_reg_write_byte(QMI8658Register_Cal1_H, &womCmd[2]);

	bsp_qmi8658_doCtrl9Command(QMI8658_Ctrl9_Cmd_WoM_Setting);
	bsp_qmi8658_enableSensors(QMI8658_CTRL7_ACC_ENABLE);
...
void bsp_qmi8658_disableWakeOnMotion(void)
{
	bsp_qmi8658_enableSensors(QMI8658_CTRL7_DISABLE_ALL);
    uint8_t data = 0;
	bsp_qmi8658_reg_write_byte(QMI8658Register_Cal1_L, &data);
	// bsp_qmi8658_doCtrl9Command(QMI8658_Ctrl9_Cmd_WoM_Setting);
}

void bsp_qmi8658_enableSensors(unsigned char enableFlags)
{
    ...
    uint8_t data = enableFlags & QMI8658_CTRL7_ENABLE_MASK;
	bsp_qmi8658_reg_write_byte(QMI8658Register_Ctrl7, &data);
}

void bsp_qmi8658_config_apply(struct QMI8658Config const *config)
{
	unsigned char fisSensors = config->inputSelection;

	if (fisSensors & QMI8658_CONFIG_AE_ENABLE)
	{
		bsp_qmi8658_config_ae(config->aeOdr);
	}
	else
	{
		if (config->inputSelection & QMI8658_CONFIG_ACC_ENABLE)
		{
			bsp_qmi8658_config_acc(config->accRange, config->accOdr, QMI8658Lpf_Enable, QMI8658St_Disable);
		}
		if (config->inputSelection & QMI8658_CONFIG_GYR_ENABLE)
		{
			bsp_qmi8658_config_gyro(config->gyrRange, config->gyrOdr, QMI8658Lpf_Enable, QMI8658St_Disable);
		}
	}

	if (config->inputSelection & QMI8658_CONFIG_MAG_ENABLE)
	{
		bsp_qmi8658_config_mag(config->magDev, config->magOdr);
	}
	bsp_qmi8658_enableSensors(fisSensors);
}
...
unsigned char bsp_qmi8658_init(void)
{
	...
	while (iCount < 2)
	{
		...
		while ((QMI8658_chip_id != 0x05) && (retry++ < 5))
		{
			...
			bsp_qmi8658_reg_read(QMI8658Register_WhoAmI, &QMI8658_chip_id, 1);
	...
	bsp_qmi8658_reg_read(QMI8658Register_Revision, &QMI8658_revision_id, 1);
	if (QMI8658_chip_id == 0x05)
	{
        ...
        uint8_t data = 0x60;
		bsp_qmi8658_reg_write_byte(QMI8658Register_Ctrl1, &data);
		...
		bsp_qmi8658_config_apply(&QMI8658_config);
		if (1)
		{
			unsigned char read_data = 0x00;
			bsp_qmi8658_reg_read(QMI8658Register_Ctrl1, &read_data, 1);
			QMI8658_printf("QMI8658Register_Ctrl1=0x%x \n", read_data);
			bsp_qmi8658_reg_read(QMI8658Register_Ctrl2, &read_data, 1);
			QMI8658_printf("QMI8658Register_Ctrl2=0x%x \n", read_data);
			bsp_qmi8658_reg_read(QMI8658Register_Ctrl3, &read_data, 1);
			QMI8658_printf("QMI8658Register_Ctrl3=0x%x \n", read_data);
			bsp_qmi8658_reg_read(QMI8658Register_Ctrl4, &read_data, 1);
			QMI8658_printf("QMI8658Register_Ctrl4=0x%x \n", read_data);
			bsp_qmi8658_reg_read(QMI8658Register_Ctrl5, &read_data, 1);
			QMI8658_printf("QMI8658Register_Ctrl5=0x%x \n", read_data);
			bsp_qmi8658_reg_read(QMI8658Register_Ctrl6, &read_data, 1);
			QMI8658_printf("QMI8658Register_Ctrl6=0x%x \n", read_data);
			bsp_qmi8658_reg_read(QMI8658Register_Ctrl7, &read_data, 1);
