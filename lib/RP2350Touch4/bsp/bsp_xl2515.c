...
#define BSP_XL2515_SPI_PORT spi0
#define BSP_XL2515_SCLK_PIN 2
...
static void xl2515_write_reg(uint8_t reg, uint8_t *data, uint8_t len)
{
    ...
    gpio_put(BSP_XL2515_CS_PIN, 0);
    spi_write_blocking(BSP_XL2515_SPI_PORT, buf, len + 2);
...
static void xl2515_read_reg(uint8_t reg, uint8_t *data, uint8_t len)
{
    ...
    spi_read_blocking(BSP_XL2515_SPI_PORT, 0, data, len);
    gpio_put(BSP_XL2515_CS_PIN, 1);
}
...
static void xl2515_write_reg_byte(uint8_t reg, uint8_t byte)
{
    ...
    gpio_put(BSP_XL2515_CS_PIN, 0);
    spi_write_blocking(BSP_XL2515_SPI_PORT, &cmd, 1);
    spi_write_blocking(BSP_XL2515_SPI_PORT, &reg, 1);
    spi_write_blocking(BSP_XL2515_SPI_PORT, &byte, 1);
    gpio_put(BSP_XL2515_CS_PIN, 1);
}
...
void bsp_xl2515_reset(void)
{
    ...
    gpio_put(BSP_XL2515_CS_PIN, 0);
    spi_write_blocking(BSP_XL2515_SPI_PORT, &buf, 1);
    gpio_put(BSP_XL2515_CS_PIN, 1);
}
...
void bsp_xl2515_init(xl2515_rate_kbps_t rate_kbps)
{
    ...
    spi_init(BSP_XL2515_SPI_PORT, 10 * 1000 * 1000);
    gpio_set_function(BSP_XL2515_SCLK_PIN, GPIO_FUNC_SPI);
    gpio_set_function(BSP_XL2515_MOSI_PIN, GPIO_FUNC_SPI);
    gpio_set_function(BSP_XL2515_MISO_PIN, GPIO_FUNC_SPI);

    gpio_init(BSP_XL2515_CS_PIN);
    gpio_init(BSP_XL2515_INT_PIN);

    gpio_set_dir(BSP_XL2515_CS_PIN, GPIO_OUT);
    gpio_set_dir(BSP_XL2515_INT_PIN, GPIO_IN);
    gpio_pull_up(BSP_XL2515_INT_PIN);
    gpio_set_irq_enabled_with_callback(BSP_XL2515_INT_PIN, GPIO_IRQ_EDGE_FALL | GPIO_IRQ_EDGE_RISE, true, gpio_callback);
